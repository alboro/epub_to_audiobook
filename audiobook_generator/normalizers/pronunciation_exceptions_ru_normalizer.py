...
BUILTIN_PRONUNCIATION_EXCEPTIONS = {
    "отель": "отэль",
    "отеля": "отэля",
    "отелю": "отэлю",
    "отелем": "отэлем",
...
class PronunciationExceptionsRuNormalizer(BaseNormalizer):
    STEP_NAME = "pronunciation_exceptions_ru"

    def __init__(self, config: GeneralConfig):
        self.replacements = BUILTIN_PRONUNCIATION_EXCEPTIONS.copy()
    ...
    def normalize(self, text: str, chapter_title: str = "") -> str:
        if not is_russian_language(self.config.language):
            logger.info(
                "pronunciation_exceptions_ru skipped for chapter '%s' because language is '%s'",
                chapter_title,
        ...
        logger.info(
            "pronunciation_exceptions_ru normalizer applied to chapter '%s': %s replacements",
            chapter_title,
            replacements,
        )
        return normalized
